...
static unsigned long g_bytes_read = 0, g_bytes_write = 0;
...
int main(int argc, char *argv[])
{
    ...
    CLI_OUT("Run test on serial ports: %s<==>%s (%d %d%c%d) ...\n",
        param.device1, param.device2, param.baudrate, param.databits,
        parity_name[param.parity], param.stopbits);
    ...
    thread_arg_t arg_write, arg_read;
    arg_write.fd = fd1;
    arg_write.data_buf = obuf;
    arg_write.loop = param.loop_count;

    arg_read.fd = fd2;
    arg_read.data_buf = cmp_buf;
    arg_read.loop = param.loop_count;

    long rc_write = 0;
    long rc_read = 0;
    pthread_t thr_id_read, thr_id_write;

    /* Start threads to do Read&Verify/Write */
    pthread_create(&thr_id_read, NULL, routine_read, &arg_read);
    pthread_create(&thr_id_write, NULL, routine_write, &arg_write);

    /* Wait until threads exit. */
    pthread_join(thr_id_write, (void *)&rc_write);
    pthread_join(thr_id_read, (void *)&rc_read);

    CLI_OUT("\n"
        "---------------------------------------------------\n"
        "Sent bytes: %lu, received bytes: %lu\n"
        "---------------------------------------------------\n",
        g_bytes_write, g_bytes_read);
...
int parse_argument(int argc, char *argv[], port_param_t *param)
{
    ...
    while ((opt = getopt(argc, argv, ":b:d:c:s:l:h")) != -1) {
        switch (opt) {
        case 'b':
    ...
    DBG_PRINT("%s %s<==>%s (%d %d%c%d) (loop %d)\n",
        pname, param->device1, param->device2, param->baudrate, param->databits,
        parity_name[param->parity], param->stopbits, param->loop_count);
